...
def classify_user_intent(message: str):
    INTENT_KEYWORDS = {
        "MARKET": [
            ...
            "earnings", "dividend", "etf", "index",
        ],
        "GREETING": ["hello", "hi", "hey", "lucy", "morning", "help"],
        "GLOBAL_KEYWORDS": [
            "market", "everything", "all stocks", "overall", "crypto world",
            # Top-performer / screener queries that need live search
            "performing", "performance", "top stocks", "best stocks",
            "gaining", "gainer", "gainers", "movers", "trending",
            "which stock", "what stock", "recommend", "hot right now",
        ]
    }
...
def generate_earnings_intelligence(symbol: str, db) -> dict:
    """
    Synthesizes pricing, jobs, web traffic, and developer activity to predict earnings sentiment.
    """
    # 1. Fetch web traffic and job openings
    alt_data = db.query(AlternativeData).filter(AlternativeData.symbol == symbol).order_by(AlternativeData.timestamp.desc()).first()
    job_postings = alt_data.job_postings if alt_data else 0
    web_traffic = alt_data.web_traffic_visits if alt_data else 0
    
    # 2. Fetch recent whale behavior
    behavior = mine_investor_behavior(db, symbol)
    
    # 3. Formulate sentiment and summary
    score = 0
    if job_postings > 20: score += 1
    if web_traffic > 1000: score += 1
    if "Accumulation" in behavior: score += 2
    if "Distribution" in behavior: score -= 2
    
    if score >= 2:
        sentiment = "Bullish"
        confidence = 0.88
        summary = f"Strong indicator: Job openings ({job_postings}) and web traffic ({web_traffic}) are expanding alongside heavy whale accumulation."
    elif score < 0:
        sentiment = "Bearish"
        confidence = 0.75
        summary = f"Cautious indicator: Whale distribution detected alongside flat or declining web traffic signals."
    else:
        sentiment = "Neutral"
        confidence = 0.50
        summary = "Web traffic and developer signals remain in a standard range. No extreme pre-earnings signals detected."
...
def generate_sector_intelligence(sector_name: str, db) -> dict:
    """
    Fuses regulatory updates, hardware pricing index, and vendor risk to output sector-level reports.
    """
    # 1. Query regulatory alerts
    reg_alerts = db.query(RegulatoryAlert).order_by(RegulatoryAlert.timestamp.desc()).limit(3).all()
    reg_severity = "Low"
    if any(a.severity == "High" or a.severity == "Critical" for a in reg_alerts):
        reg_severity = "High"
        
    # 2. Query hardware GPU competitive pricing index
    prices = db.query(CompetitivePricing).filter(CompetitivePricing.item_name.like("%RTX 4090%")).all()
    avg_gpu_price = sum(p.price for p in prices) / len(prices) if prices else 1749.99
    
    # 3. Query vendor risk instances
    risks = db.query(CorporateRisk).order_by(CorporateRisk.timestamp.desc()).limit(2).all()
    risk_count = len(risks)
    
    # Sector outlook
    outlook = "Positive" if reg_severity == "Low" and risk_count == 0 else "Cautious"
    
    report = {
        "sector": sector_name,
        "regulatory_impact": reg_severity,
        "regulatory_updates": [a.title for a in reg_alerts],
        "hardware_index_price": round(avg_gpu_price, 2),
        "vendor_risk_signals": [f"{r.company_name}: {r.leadership_signals} ({r.financial_health})" for r in risks],
        "outlook": outlook
    }
    
    # Cache to database
    import json
    cached = SectorIntelligence(
        sector_name=sector_name,
        synthesis_json=json.dumps(report)
    )
    db.add(cached)
    db.commit()
