...
import type { AddressLike, ContractTransactionResponse } from "ethers";
import { network } from "hardhat";
import type { SeedToken } from "../types/ethers-contracts/index.js";
import { Contract, Web3, Web3Context } from 'web3';
import type { Abi } from "hardhat/types/artifacts";

const web3 = new Web3('https://rpc.sepolia.org');
const web3Context = new Web3Context(web3.currentProvider as any);
...
describe("SeedToken Test", function () {
    ...
    let deployer: { sendTransaction: (arg0: { from: AddressLike; to: AddressLike; value: bigint; }) => any; address: AddressLike; };
    let deployer_: any;
 
    let seedToken: SeedToken & { deploymentTransaction(): ContractTransactionResponse; };
    let seedToken_: Contract<Abi>;
    ...
    it("does not allow anyone else to set a new owner", async function () {
        ...
        try {
            const tx = {
                ...
                to: seedToken_.options.address as string,
                gas: 1000000,
    ...
    it("allows the owner to mint tokens", async function () {
        ...
        const decimals_ = BigInt(await seedToken_.methods.decimals().call() as string);
        ...
        const originalAmount_ = 
            BigInt(await seedToken_.methods.totalSupply().call() as string) / divisor_;
